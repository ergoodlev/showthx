 ...
 * @param {boolean} requireAuth - Whether to require biometric auth before enabling (default: false)
 */
export const enableBiometricLogin = async (userId, email = null, password = null, requireAuth = false) => {
  try {
    // Optionally authenticate to confirm identity
    // We skip this by default when enabling after a successful password login
    // since the user just proved their identity
    if (requireAuth) {
