-- ========================================
-- GratituGram Security & Approval Schema
-- Implements child safety, data retention, audit logging
-- ========================================

-- ========================================
-- 1. Update Videos Table with Approval Workflow
-- ========================================

ALTER TABLE IF EXISTS videos
ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'draft' CHECK (status IN ('draft', 'approved', 'sent')),
ADD COLUMN IF NOT EXISTS approved_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS approved_by UUID REFERENCES auth.users(id),
ADD COLUMN IF NOT EXISTS sent_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS expires_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS video_type TEXT DEFAULT 'thank_you' CHECK (video_type IN ('thank_you', 'gift_opening', 'combined'));

-- Create index for status-based queries (performance)
CREATE INDEX IF NOT EXISTS idx_videos_status ON videos(status);
CREATE INDEX IF NOT EXISTS idx_videos_expires_at ON videos(expires_at);

-- ========================================
-- 2. Add Children Table for Multi-Child Support
-- ========================================
...
-- Add child_id foreign key to videos if not exists
ALTER TABLE IF EXISTS videos
...
-- ========================================
-- 4. Audit Logs Table (for COPPA compliance)
-- ========================================
...
-- ========================================
-- 5. Parental Consent & Preferences Table
-- ========================================

CREATE TABLE IF NOT EXISTS parental_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL UNIQUE REFERENCES auth.users(id) ON DELETE CASCADE,
  parent_pin TEXT NOT NULL, -- Should be hashed in application layer
...
-- ========================================
-- 7. Storage Policies (if needed)
-- ========================================

-- Protect video storage bucket
-- IMPORTANT: Run these in Supabase dashboard under Storage > Policies
-- OR use the Supabase client with admin privileges

-- SELECT policy: Allow users to view their own videos
-- user_id = auth.uid()

-- INSERT policy: Allow users to upload their own videos
-- user_id = auth.uid()

-- DELETE policy: Allow users to delete their own videos
-- user_id = auth.uid()

-- ========================================
-- 8. Functions for Automatic Cleanup
-- ========================================
